...
const useStyles = makeStyles({
  pageContainer: {
    ...
    padding: "24px", backgroundColor: tokens.colorNeutralBackground1,
    background: `radial-gradient(ellipse at top, rgba(0,120,212,0.05), transparent 60%),
                 radial-gradient(ellipse at bottom, rgba(0,120,212,0.08), transparent 55%), #FAFAFA`,
  },
  roleSelectionContainer: { maxWidth: "900px", width: "100%" },
  logoContainer: { textAlign: "center", marginBottom: "40px" },
  logoIcon: {
    width: "40px", height: "40px", borderRadius: "4px", backgroundColor: tokens.colorBrandBackground,
    display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px",
  },
  title: { fontSize: tokens.fontSizeHero700, fontWeight: tokens.fontWeightSemibold, color: tokens.colorNeutralForeground1, marginBottom: "8px", lineHeight: tokens.lineHeightHero700 },
  subtitle: { fontSize: tokens.fontSizeBase300, color: tokens.colorNeutralForeground3 },
  roleGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "16px" },
  roleCard: {
    backgroundColor: tokens.colorNeutralBackground1, borderRadius: "4px", border: `1px solid ${tokens.colorNeutralStroke2}`,
    padding: "24px", cursor: "pointer", textAlign: "center", boxShadow: tokens.shadow2,
    ":hover": { boxShadow: tokens.shadow8 },
  },
  roleIconContainer: {
    width: "48px", height: "48px", borderRadius: "4px", backgroundColor: tokens.colorBrandBackground,
    display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px",
  },
  roleTitle: { fontSize: tokens.fontSizeBase500, fontWeight: tokens.fontWeightSemibold, color: tokens.colorNeutralForeground1, marginBottom: "8px", lineHeight: tokens.lineHeightBase500 },
  roleDescription: { fontSize: tokens.fontSizeBase300, color: tokens.colorNeutralForeground3 },
  loginContainer: { maxWidth: "450px", width: "100%" },
  backButton: { marginBottom: "24px", fontSize: tokens.fontSizeBase200 },
  loginCard: {
    backgroundColor: tokens.colorNeutralBackground1, borderRadius: "4px", border: `1px solid ${tokens.colorNeutralStroke2}`,
    padding: "44px", boxShadow: tokens.shadow16, width: "100%", maxWidth: "440px",
  },
  loginHeaderIcon: {
    width: "56px", height: "56px", borderRadius: "12px", backgroundColor: tokens.colorBrandBackground,
    display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px",
  },
  ...
  advisorCard: {
    padding: "16px", border: `1px solid ${tokens.colorNeutralStroke2}`, borderRadius: "4px",
    cursor: "pointer", textAlign: "center", backgroundColor: tokens.colorNeutralBackground1,
...
export function Login() {
  const navigate = useNavigate(); const styles = useStyles();
  ...
  if (!role) return (
    <div className={styles.pageContainer}><div className={styles.roleSelectionContainer}>
      <div className={styles.logoContainer}><div className={styles.logoIcon}><Eye20Regular style={{ width: "20px", height: "20px", color: "white" }} /></div>
        <h1 className={styles.title}>VeraVisión CRM</h1><p className={styles.subtitle}>Sistema de gestión de clientes para óptica</p>
      </div>
      <div className={styles.roleGrid}>
        <div className={styles.roleCard} onClick={() => setRole("asesora")}>
          ...
          <h2 className={styles.roleTitle}>Soy Asesora</h2><p className={styles.roleDescription}>Accede a tu cola de tareas, calendario y pacientes asignados</p>
        </div>
        <div className={styles.roleCard} onClick={() => setRole("gerente")}>
          ...
          <h2 className={styles.roleTitle}>Soy Gerente</h2><p className={styles.roleDescription}>Supervisa el equipo, estadísticas y configura el sistema</p>
        </div>
      </div>
    </div></div>
  );
  ...
  if (role === "gerente") return (
    <div className={styles.pageContainer}><div className={styles.loginContainer}>
      ...
      <Card className={styles.loginCard}>
        ...
        <h2 className={styles.loginTitle}>Acceso Gerencia</h2><p className={styles.loginSubtitle}>Ingresa tu código de gerencia</p>
        <div className={styles.formGroup}>
          <div><Label size="small" weight="semibold">Código de Gerencia</Label>
            <Input type="password" value={code} onChange={(e) => { setCode(e.target.value); setError(""); }} onKeyDown={(e) => e.key === "Enter" && handleLogin()} placeholder="Ingresa tu código" size="large" className={styles.fullWidthInput} />
          </div>
  ...
  if (role === "asesora" && !selectedAdvisor) return (
    <div className={styles.pageContainer}><div style={{ maxWidth: "600px", width: "100%" }}>
      <Button appearance="subtle" onClick={() => { setRole(null); setCode(""); setError(""); }} className={styles.backButton}>← Volver</Button>
      <Card className={styles.loginCard} style={{ maxWidth: "600px" }}>
        ...
        <h2 className={styles.loginTitle}>Selecciona tu Usuario</h2><p className={styles.loginSubtitle}>Elige tu nombre de la lista</p>
  ...
  return (
    <div className={styles.pageContainer}><div className={styles.loginContainer}>
      ...
      <Card className={styles.loginCard}>
        ...
        <h2 className={styles.loginTitle}>{selectedAdvisor.nombre}</h2><p className={styles.loginSubtitle}>Ingresa tu código de acceso</p>
        <div className={styles.formGroup}>
          <div><Label size="small" weight="semibold">Código de Acceso</Label>
            <Input type="password" value={code} onChange={(e) => { setCode(e.target.value); setError(""); }} onKeyDown={(e) => e.key === "Enter" && handleLogin()} placeholder="Ingresa tu código" size="large" className={styles.fullWidthInput} />
          </div>
          {error && <MessageBar intent="error" icon={<ErrorCircle20Regular />}><MessageBarBody>{error}</MessageBarBody></MessageBar>}
          <Button appearance="primary" onClick={handleLogin} disabled={!code} size="large" className={styles.fullWidthButton}>Iniciar Sesión</Button>
